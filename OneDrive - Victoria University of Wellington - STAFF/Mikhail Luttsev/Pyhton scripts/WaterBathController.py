...
class TemperatureLogger:
    def __init__(self, controller, logger, filename = 'temperature_log.csv', interval=60, period_days= 60, stop_event = None):
    ...
    def periodic_request(self):
        try:
            for _ in range((self.period_days*24*60*60)//self.interval+1):
                if self.stop_event and self.stop_event.is_set():
                    self.logger.info('Cycle stopped manually')
                    break                
                temperature = self.get_temperature()
                self.write_temperature_to_file(temperature)
                self.logger.info(f'Succesfully written temperature at {datetime.datetime.now()}. Next request in {self.interval} sec')
                if self.stop_event:
...
class TemperatureCycle:
    def __init__(self, lower_temp, higher_temp, logger, controller =  'WaterBathController', interval_hours = 12, period_days = 60, stop_event = None):
    ...
    def start(self):
        ...
        self.logger.info(f'Succesfully start temperature cycle. Period of cycling {self.period_days} days. Interval - {self.interval_hours} hours.')        
        ...
        try:
            for _ in range(int(period_days*24//interval_hours+1)):
                if self.stop_event and self.stop_event.is_set():
                    ...
                    break
                self.change_temp()
                self.logger.info(f'Setting water bath temperature to {self.curr_temp}')
                self.set_controller_temperature(self.curr_temp)
                self.logger.info(f'Water bath temperature has been changed at {datetime.datetime.now()}. Next temperature cahnge in {self.interval_hours} hours')
                if self.stop_event:
...
class WaterBathController:
    ...
    def set_command(self, command):
        ...
        try:    
             ...
             self.logger.info(f'Send commend {cmd_bytes} to water bath. Get response {response}') 
             return response
        except Exception as e:
    ...
    def set_water_bath_temperature(self, temperature):
        ...
        while not self.is_connected and attemps > 0:
            self.logger.info(f'No connection. Try to set connection. Attempts remaining {attemps}')
            self.connect()
            attemps -= 1
            time.sleep(0.1)
        if self.is_connected:
            ...
            self.logger.info(f'Succesfully set water bath temperature - {temperature}')
    def get_water_bath_temperature(self):
        ...
        if self.is_connected:
            ...
            try:
                ...
                self.logger.info(f'Succesfully got water bath temperature - {float(temperature)}')
